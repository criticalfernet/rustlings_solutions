...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        let iter = [tuple.0,tuple.1,tuple.2].into_iter();

        if iter.filter(|p| *p >= 0 && *p < 256).count() != 3 {return Err(Self::Error::IntConversion);}
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        let iter = arr.iter();
        if iter.filter(|p| **p < 255 && **p >= 0).count() != 3 {return Err(Self::Error::IntConversion);}
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        if iter.len() != 3 {return Err(Self::Error::BadLen);}
